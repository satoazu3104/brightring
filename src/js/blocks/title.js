...
registerBlockType('portart/title', {
  ...
  save: ({ attributes: { titleEn, titleJp, addClassName, addId, linkUrl, isChecked, } }) => {
    const Jp = () => {
      if (titleJp) {
        return (
          <div className='p-title__wrap--jp'>
            <RichText.Content
              value={titleJp}
              tagName='p'
              className='c-text__section-title-jp animation c-text--bold js-blocks-text'
            />
          </div>
        )
      }
    ...
    const En = () => {
      if (titleEn) {
        return (
          <div className={ linkUrl ? 'p-title__wrap--en link' : 'p-title__wrap--en'}>
            <RichText.Content
              className='c-text__section-title c-text--med c-text--en animation'
              value={titleEn}
              tagName='h3'
            />
          </div>
